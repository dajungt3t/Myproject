<?php
    class jangbuo extends CI_Controller {                                //jangbuo 클래스 선언
        function __construct() {                                         //클래스 생성 시 초기 선언
            parent::__construct();                             
            $this->load->database();                                     //DB연결                           
            $this->load->model("jangbuo_m");                             //model jangbuo_M.php 연결
        ...
        public function lists() {                                       //리스트, 사융자 검색
            ...
            $base_url = "/jangbuo/lists/text1/$text1";

            $data["text1"]=$text1;                                      //검색

            $data["list"] = $this->jangbuo_m->getlist($text1);          //목록 읽기

            $this->load->view("main_header");                           //상단메뉴 출력
            if($this->session->userdata('id')=="") {                    //로그인 상태에서만 리스트 확인
                $this->load->view("main_home", $data);
            } else {   
                $this->load->view("jangbuo_list", $data);                   //jangbuo_list에 자료 전달
            }
        ...
        public function view() {                                        //사용자보기
            ...
            $date["text1"]=$text1;
            $data["row"]=$this->jangbuo_m->getrow($no);                 //jangbuo_m의 getrow()함수 호출 하여 $data["row"]에 값 저장

            $this->load->view("main_header");                   
            $this->load->view("jangbuo_view", $data);                   //jangbuo_view에 자료 전달
        ...
        public function del() {                                         //사용자 삭제
            ...
            redirect("/ci/jangbuo/lists/" .$text1);                     //목록으로 돌아가기
        }
        ...
        public function add() {                                         //사용자 추가
            ...
            if($this->form_validation->run()==FALSE) {
                $data["list"] = $this->jangbuo_m->getlist_product();      //구분 리스트 불러오기
                ...
                redirect("/ci/jangbuo/lists".$text1);
            }
        }
        ...
        public function edit() {                                       //사용자 수정
            ...
            if($this->form_validation->run()==FALSE) {
                ...
                $this->load->view("main_header");                   
                $data["row"]=$this->jangbuo_m->getrow($no);           //사용자 정보 가져오기
                ...
                $result = $this->jangbuo_m->updaterow($data, $no);        //데이터 저장
