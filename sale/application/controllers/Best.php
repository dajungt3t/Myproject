<?php
    class Best extends CI_Controller {                                //best 클래스 선언
        function __construct() {                                         //클래스 생성 시 초기 선언
            parent::__construct();                             
            $this->load->database();                                     //DB연결                           
            $this->load->model("best_m");                             //model best_M.php 연결
        ...
        public function lists() {                                       //리스트, 사융자 검색
            ...
            $base_url = "/best/lists/text1/$text1/text2/$text2";

            $data["text1"]=$text1;   
            $data["text2"]=$text2;                                  

            $data["list"] = $this->best_m->getlist($text1, $text2);          //목록 읽기

            $this->load->view("main_header");                           //상단메뉴 출력
            if($this->session->userdata('id')=="") {                    //로그인 상태에서만 리스트 확인
                $this->load->view("main_home", $data);
            } else {   
                $this->load->view("best_list", $data);                   //best_list에 자료 전달
            }
